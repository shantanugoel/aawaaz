...
final class CleanupQualityTests: XCTestCase {
    ...
    private func printSummary(results: [CaseResult]) {
        ...
        for category in sortedCategories {
            ...
            print("\(pad(category, 25)) \(pad("\(count)", 5)) \(pad("\(passCount)", 5)) \(pad("\(failCount)", 5)) \(pad(String(format: "%.2fs", avgLLMLatency), 11))")
        }

        let overallAvgLLM = totalCount > 0 ? totalLLMLatency / Double(totalCount) : 0
        print(String(repeating: "─", count: 55))
        print("\(pad("TOTAL", 25)) \(pad("\(totalCount)", 5)) \(pad("\(totalPassed)", 5)) \(pad("\(totalFailed)", 5)) \(pad(String(format: "%.2fs", overallAvgLLM), 11))")
    }
